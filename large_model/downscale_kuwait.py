...
def get_area_data(down_g_data, season, n_lag):
    target_var = 'DUEXTTAU'
    season = int(season)
    ...
    file_path_country = ['/project/mereditf_284/menglin/Downscale_data/Country_shape/KWT_adm/KWT_adm0.shp']
...
def get_true_data(test_set):
    ...
    g_data, m_data, [G_lats, G_lons, M_lats, M_lons], ele_data = data_processor.load_data(target_var,
                                                                                          ...
                                                                                          file_path_country)
    match_m_data_all = data_processor.unify_m_data(g_data, m_data, G_lats, G_lons, M_lats, M_lons)
    match_m_data_all = match_m_data_all[(365*7 - len(test_set)):]
    return g_data[test_set, :, :], match_m_data_all, [G_lats, G_lons, M_lats, M_lons], ele_data
...
def index_to_season(index):
    season = int((index%365)/91)
    if season in [0, 1, 2]:
        return season +1
    else:
        return 4
...
if __name__ == "__main__":
    ...
    # construct init data
    down_g_data, match_m_data_all, [G_lats, G_lons, M_lats, M_lons], ele_data = get_true_data(list(range(-n_lag, 0)))
